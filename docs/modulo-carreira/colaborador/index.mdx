...
**Hub vs páginas de detalhe.** Esta página sintetiza a jornada na ordem das perguntas do utilizador. Cada outra entrada cobre **uma superfície do produto** (rota ou separador) e usa `<AnaliseProduto>` para decisões, hipóteses e fronteiras, e `<ReferenciaProtótipo>` para *copy* e elementos observados no mock.
